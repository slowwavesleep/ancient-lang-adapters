...
import evaluate
import numpy as np
from adapters import AutoAdapterModel, AdapterTrainer, AdapterConfig, AdapterArguments
...
class DataTrainingArguments:
    ...
    force_num_labels: Optional[int] = field(default=None, metadata={"help": "Force specific number of labels"})
...
class ModelArguments:
    ...
    mlm_adapter_path: str = field(
        default=None,
        metadata={"help": "Path to MLM adapter"},
    )
...
def main():
    ...
    if data_args.force_num_labels is not None:
        num_labels = data_args.force_num_labels
    ...
    if model_args.mlm_adapter_path is not None:
    ...
    if model_args.task_adapter_path is not None:
        print("Loading trained task adapter")
        task_adapter_path = (Path(model_args.task_adapter_path) / "polarity").resolve()
        task_adapter_config_path = task_adapter_path / "adapter_config.json"
        task_adapter_config = AdapterConfig.load(task_adapter_config_path.as_posix())
        model.load_adapter(task_adapter_path.as_posix(), config=task_adapter_config)
    else:
        task_adapter_config = AdapterConfig.load(adapter_args.adapter_config)
        model.add_adapter(data_args.task_name, config=task_adapter_config)
    ...
    if model_args.mlm_adapter_path is not None:
        model.active_adapters = adapters.composition.Stack("mlm", data_args.task_name)
    else:
    ...
    if training_args.do_train:
        ...
        output_labels_file = os.path.join(
            training_args.output_dir, f"{data_args.task_name}_labels.json"
        )
        with open(output_labels_file, "w") as file:
            file.write(json.dumps(label_to_id))
