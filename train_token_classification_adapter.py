...
def main():
    ...
    if labels_are_int:
        ...
        label_list = get_label_list(raw_datasets["train"][label_column_name]) + ["UNK"]
    ...
    adapter_config = AdapterConfig.load(adapter_config_path.as_posix())
    ...
    if model_args.tokenizer_name:
        model.load_embeddings(
            (Path(model_args.mlm_adapter_path) / "embeddings").as_posix(),
            "custom_embeddings",
        )
    model.load_adapter(mlm_adapter_path.as_posix(), config=adapter_config)
    model.add_adapter(task_name)
    model.add_tagging_head(task_name, num_labels=num_labels)
    model.train_adapter([task_name])
    model.active_adapters = ac.Stack("mlm", task_name)
        ...
        for i, label in enumerate(examples[label_column_name]):
            ...
            for word_idx in word_ids:
                ...
                # We set the label for the first token of each word.
                    ...
                    label_ids.append(
                        label_to_id.get(label[word_idx], label_to_id["UNK"])
                    )
    ...
    if training_args.do_train:
        ...
        output_labels_file = os.path.join(
            training_args.output_dir, f"{task_name}_labels.json"
        )
        with open(output_labels_file, "w") as file:
            file.write(json.dumps(label_to_id))
        if model_args.tokenizer_name:
            model.save_embeddings(
                Path(training_args.output_dir) / "embeddings", "custom_embeddings"
            )
